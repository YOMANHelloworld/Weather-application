...
class DateFormatterUtils {
    ...
    static func formattedDateTime(from timestamp: TimeInterval, timeZoneIdentifly: String) -> String {
        let date = Date(timeIntervalSince1970: timestamp)
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "d MMM yyyy 'at' h:mm a"
        
        if let timeZone = TimeZone(identifier: timeZoneIdentifly) {
            dateFormatter.timeZone = timeZone
        } else {
            print("Invalid time zone: \(timeZoneIdentifly)")
        }
